...
const COLUMNS = ["name", "email", "password", "phone", "perClientFee", "bio"];
const SAMPLE_ROW = ["Jane Smith", "jane@example.com", "password123", "9876543210", "500", "Specializes in weight training"];
...
export default function BulkUploadTrainersPage() {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link href="/admin/trainers"><ArrowLeft className="h-4 w-4 mr-1" /> Back</Link>
        </Button>
        <div>
          <h1 className="text-2xl font-bold">Bulk Upload Trainers</h1>
          <p className="text-muted-foreground text-sm">Upload a CSV file to add multiple trainers at once.</p>
        </div>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm text-muted-foreground">
          <p>Download the template CSV, fill in your trainer data, then upload it here.</p>
          <ul className="list-disc list-inside space-y-1">
            ...
            <li><strong>perClientFee</strong> — Fee per client in ₹/month (optional, integer)</li>
            <li><strong>bio</strong> — Short bio (optional)</li>
          </ul>
          <p className="text-amber-600 dark:text-amber-400 text-xs">
            Note: Rows that would exceed your plan&apos;s trainer limit will be skipped.
          </p>
          ...
          {rows.length > 0 && !results && (
            <div className="space-y-3">
              ...
              <Button onClick={handleUpload} disabled={isUploading}>
                {isUploading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Uploading…</> : <><Upload className="h-4 w-4 mr-2" /> Upload {rows.length} Trainers</>}
              </Button>
            </div>
          )}
        </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button asChild>
                <Link href="/admin/trainers">View All Trainers</Link>
              </Button>
